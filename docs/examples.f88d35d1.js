...
var NATIVE_EVENTS = exports.NATIVE_EVENTS = {
  CHANGE: 'change',
  CLICK: 'click',
...
var CUSTOM_EVENTS = exports.CUSTOM_EVENTS = {
  DESTROY: 'destroy',
...
var DropArea = function (_Emitter) {
  ...
  function DropArea(element) {
    ...
    var _this = _possibleConstructorReturn(this, (DropArea.__proto__ || Object.getPrototypeOf(DropArea)).call(this));
  ...
  _createClass(DropArea, [{
    ...
    value: function destroy() {
      if (this._element) {
        ...
        this.emit(_events.CUSTOM_EVENTS.DESTROY);
      }
    }
    ...
    value: function handleEvent(event) {
      if (!this._element.isSameNode(event.target) && !this._element.contains(event.target) && !this._targetShadowDOMContainsDropArea(event)) {
        return;
      }

      switch (event.type) {
        case _events.NATIVE_EVENTS.CLICK:
          this._onClick();
          break;
        case _events.NATIVE_EVENTS.DROP:
          event.preventDefault();
          this._onDrop(event);
          break;
        case _events.NATIVE_EVENTS.DRAG_ENTER:
        case _events.NATIVE_EVENTS.DRAG_OVER:
          event.preventDefault();
          this._onInsideDropArea();
          break;
        case _events.NATIVE_EVENTS.DRAG_END:
        case _events.NATIVE_EVENTS.DRAG_LEAVE:
    ...
    value: function _toggleListeners(toggle) {
      ...
      this._element[action](_events.NATIVE_EVENTS.CLICK, this);
      this._element[action](_events.NATIVE_EVENTS.DRAG_START, this);
      this._element[action](_events.NATIVE_EVENTS.DRAG_END, this);
      this._element[action](_events.NATIVE_EVENTS.DRAG_ENTER, this);
      this._element[action](_events.NATIVE_EVENTS.DRAG_LEAVE, this);

      // The following events need to be bound to the window to successfully prevent default actions
      window[action](_events.NATIVE_EVENTS.DRAG_OVER, this);
      window[action](_events.NATIVE_EVENTS.DROP, this);
    }
    ...
    value: function _configureHiddenFileInput() {
      ...
      input.addEventListener(_events.NATIVE_EVENTS.CLICK, function (event) {
        // Stops the invoked file input click bubbling and triggering parent listeners
        event.stopPropagation();
      });
      input.addEventListener(_events.NATIVE_EVENTS.CHANGE, function (event) {
    ...
    key: '_targetShadowDOMContainsDropArea',
    value: function _targetShadowDOMContainsDropArea(event) {
    ...
    value: function _onFilesAdded(files) {
      if (files.length) {
        this.emit(_events.CUSTOM_EVENTS.FILES_ADDED, files);
      }
    }
  }]);
...
},{"tiny-emitter":17,"./events":14}],16:[function(require,module,exports) {
'use strict';
...
var DropFile = function () {
  ...
  _createClass(DropFile, [{
    ...
    key: '_createDataUri',
    value: function _createDataUri(file) {
...
var _dropArea = require('./drop-area');

var _dropArea2 = _interopRequireDefault(_dropArea);

var _dropFile = require('./drop-file');
...
var Ascender = function (_Emitter) {
  _inherits(Ascender, _Emitter);

  function Ascender(element) {
    _classCallCheck(this, Ascender);

    var _this = _possibleConstructorReturn(this, (Ascender.__proto__ || Object.getPrototypeOf(Ascender)).call(this));

    _this.dropArea = new _dropArea2.default(element);
    _this.files = [];
  ...
  _createClass(Ascender, [{
    ...
    value: function _toggleListeners(toggle) {
      ...
      this.dropArea[action](_events.CUSTOM_EVENTS.FILES_ADDED, function (files) {
    ...
    value: function _onFilesAdded(files) {
      for (var i = 0; i < files.length; i++) {
        var fileToAdd = new _dropFile2.default(files[i]);

        this.files.push(fileToAdd);
        this.emit(_events.CUSTOM_EVENTS.FILE_ADDED, fileToAdd);
      }
    }
  }]);
...
},{"./events":14,"tiny-emitter":17,"./drop-area":15,"./drop-file":16}],6:[function(require,module,exports) {
...
function createCodeSnippet(code, type) {
  var highlighted = _prismjs2.default.highlight(code, _prismjs2.default.languages[type], type);

  return '\n<h3 class="uppercase">' + type + '</h3>\n<pre><code class="language-' + type + '">' + highlighted + '</code></pre>\n  ';
}

function createLiveShadow(files, selector) {
  var liveExample = document.createElement('div');
  var script = document.createElement('script');
  var style = document.createElement('style');
  var html = document.createElement('div');
  var shadowRootStr = 'document.querySelector(\'' + selector + ' .live-example\').shadowRoot.querySelector';

  liveExample.classList.add('live-example');
  var shadow = liveExample.attachShadow({ mode: 'open' });
...
function renderExample(example, selector) {
  var exampleNode = document.querySelector(selector);

  exampleNode.innerHTML = createCodeSnippet(example.js, 'javascript') + createCodeSnippet(example.css, 'css') + createCodeSnippet(example.html, 'html') + '<h3 class="uppercase">Result</h3>';

  var liveExample = createLiveShadow(example, selector);
  exampleNode.appendChild(liveExample);
}
},{"prismjs":9}],3:[function(require,module,exports) {
'use strict';

var _ascender = require('../src/ascender');

var _ascender2 = _interopRequireDefault(_ascender);

var _fs = require('fs');

var _fs2 = _interopRequireDefault(_fs);

var _exampler = require('./exampler');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

window.Ascender = _ascender2.default;
...
var imagePreviewExample = {
  html: '<div class="drop-area">\n  <h1>Drop your images here or click to select</h1>\n</div>\n\n<h2>Image Previews:</h2>\n\n<div class="preview-container"></div>\n',
  css: '.drop-area {\n  width: 100%;\n  height: 200px;\n  text-align: center;\n  border: 2px dashed #000;\n}\n\nh1 {\n  padding-top: 40px;\n}\n\n/* Class added when Drag/Drop event inside Drop Area */\n.asc-drop-area--inside {\n  background-color: #ccc;\n}\n\n.image-preview {\n  max-width: 200px;\n  max-height: 200px;\n}\n',
  js: 'const dropAreaNode = document.querySelector(\'.drop-area\')\nconst previewNode = document.querySelector(\'.preview-container\')\n\nconst instance = new Ascender(dropAreaNode)\n\ninstance.on(\'file:added\', (file) => {\n  file.getDataUri().then(uri => {\n    const newImg = document.createElement(\'img\')\n    newImg.src = uri\n    newImg.classList.add(\'image-preview\')\n    previewNode.appendChild(newImg)\n  })\n})\n'
};

(0, _exampler.renderExample)(imagePreviewExample, '.image-preview-example');
},{"../src/ascender":13,"fs":6,"./exampler":7}],18:[function(require,module,exports) {
...
//# sourceMappingURL=/examples.f88d35d1.map